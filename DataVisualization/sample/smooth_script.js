var smoothedDraw = function(){
	...
	/*================================= LINE_STRIPS ============================================*/
	//POINT
	var smoothedDraw = readFile.dataLoad;
	var smoothedLine = new Array();
	for(var k=0; k < 4000; k++){
		smoothedLine[6*k] = smoothedData[2*k]*0.5;
		smoothedLine[6*k+1] = smoothedData[2*k+1]*0.5;
		smoothedLine[6*k+2] = 0;
		
		//color
		smoothedLine[6*k+3] = 0;
		smoothedLine[6*k+4] = 1;
		smoothedLine[6*k+5] = 0;
	}
	var SMOOTHEDLINE_VERTEX = GL.createBuffer();
	GL.bindBuffer(GL.ARRAY_BUFFER, SMOOTHEDLINE_VERTEX);
	GL.bufferData(GL.ARRAY_BUFFER, new Float32Array(smoothedLine), GL.STATIC_DRAW);
